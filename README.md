...
| **World item supply** | `GET fanout.kintara.gg/api/world-item-index?category=all&sort=desc` | Total units of each item across **all** player inventories/banks/bags — powers kintara.gg's `/#index`. `{ok, playerCount, generatedAt, rows:[{id (=item_type), label, icon, count, category}]}`. The authoritative origin needs a logged-in session; the **public read-fanout mirror serves it without auth** (frequently `fanout_unavailable` transiently). Refreshed by a **background loop** (`world_supply_loop`, every `WORLD_INDEX_CACHE_SEC`≈10min) that **persists each good snapshot** to the DB (`world_supply_json` setting); `world_item_supply()` is then a pure in-memory read seeded from that snapshot on boot — **never fetches on the request path**, so the Index/market-cap "In world" data is available instantly on load and survives restarts + fanout outages. **Also the live item catalog:** we capture each row's `label`, `icon` path, and `category` (`world_item_meta()`), which `item_label()` / `categorize()` / `icon_candidates()` use as the fallback — so **new items added to the game are auto-indexed** (correct name, art, category, supply) within ~10min with no code change. Drives the Index "In world" column + market cap + new-item handling. |
...
- **Auto-index new game items (names, icons, categories) from kintara's live index.** Today's update
  added a new area + items (e.g. `molten_rock`, `mount_trex`, lava cosmetics) that showed up unlabeled
  and logo-less. Root cause: our label catalog (`ITEM_LABELS`), icon-path rules, and `categorize()`
  were static and didn't know new items. Fix: kintara's own world-item-index (already fetched for
  in-world supply) carries `label`, `icon` path, and `category` per item — we now capture all three
  (`world_item_meta()`) and use them as the **fallback** in `item_label()` (correct display name),
  `categorize()` (mapped via `_WORLD_CAT_MAP`, e.g. molten_rock→material), and `icon_candidates()`
  (the real art path first, plus underscore-stripped resource guesses like `moltenrock.png`). The Index
  item universe now also unions the full live index, so a brand-new item appears immediately with its
  in-world supply even before any listing. Net: **any future item the game adds is indexed
  automatically within ~10min** (the world-supply refresh) — name, art, category, supply — no code
  change needed. Verified live on the new molten/T-Rex/lava items.
...
- **In-world supply: persisted + background-refreshed (instant on load).** Previously
  ...
  blank. Now a `world_supply_loop` refreshes in the background (~10min) and **persists every good
  snapshot** to the DB (`world_supply_json`); `world_item_supply()` is a pure in-memory read seeded
  from that snapshot on boot — never touches the network in a request. Result: "In world" + market cap
  are available the instant the site loads and survive restarts and fanout outages (verified: served
  121 items <1s after boot with the fanout down). On failure it keeps last-good and retries.
